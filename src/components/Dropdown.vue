<template>
  <component class="dropdown nav-item" :is="tag" :class="[{ show: isOpen }]" aria-haspopup="true" :aria-expanded="isOpen" @click="toggleDropDown" v-click-outside="closeDropDown">
    <a class="dropdown-toggle nav-link" :class="{ 'hide-arrow': hideArrow }" data-toggle="dropdown">
      <slot name="title">
        <i :class="icon"></i>
    ...
    <ul class="dropdown-menu" :class="[{ 'dropdown-menu-right': position === 'right' }, { show: isOpen }]">
      <slot></slot>
    </ul>
  </component>
</template>
...
@import 'src/assets/sass/spa/variables';
@import 'src/assets/sass/spa/mixins';
li.dropdown {
