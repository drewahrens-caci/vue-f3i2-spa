<template>
  <component :is="baseComponent" :to="link.path ? link.path : '/'" class="nav-item" :class="{ active: isActive }" tag="li">
    ...
    <slot name="title" v-if="children.length === 0 && !$slots.default && link.path">
      <component :to="link.path" @click.native="linkClick" :is="elementType(link, false)" :class="{ active: link.active }" class="nav-link" :target="link.target" :href="link.path" :style="sidebarItemStyle">
      ...
      default: () => {
        return {
          ...
          backgroundColor: {
            ...
            default: 'red',
            validator: value => {
              let acceptedValues = ['white', '#3399FF', '#F7D117', '#10CB31', '#FF7E14', '#ee4236', '#A5499C', '#212121']
